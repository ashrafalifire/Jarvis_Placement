...
def background_voice_listener():
    ...
    mic = sr.Microphone()
    with mic as source:
        recognizer_sr.adjust_for_ambient_noise(source)
    print("[VOICE] Background voice listener started...")
    while app_running:
        with mic as source:
            try:
                audio = recognizer_sr.listen(source, timeout=5, phrase_time_limit=5)
                command = recognizer_sr.recognize_google(audio).lower()
                if "jarvis start the music" in command:
                    print("[VOICE] Command: start music")
                    play_bgm()
                elif "jarvis mute" in command:
                    print("[VOICE] Command: stop music")
                    stop_bgm()
